...
def grind(genome_path, gtf_path, output_dir=os.getcwd(), t_flag=False, e_flag=False, c_flag=False):
    ...
    # loop through every genome in gtf (usually there is only 1 genome per GTF)
            ...
            for transcript_id in gtf[genome_id][gene_id]:
                ...
                for exon in exons:
                    ...
                    if p0 not in tmp_exons_dict:
                        ...
                        tmp_exons_dict[p0] = genome[genome_id][p0:pf + 1]
                    else:
                        raise Exception(
    ...
    for genome_id in transcript_struct:
    ...
    for exon_key, exon_val in exon_struct:
        p0,pf = exon_key.split(";")
        exome_output += ">gene_id={0} length={1} {2}\n".format(exon_val["gene_id"], str(pf-p0+1), "".join(exon_val["transcripts"]))
        exome_output += genome[list(genome)[0]][p0-1:pf-1]
